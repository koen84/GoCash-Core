...
pkgdatadir = $(datadir)/gcash
pkgincludedir = $(includedir)/gcash
pkglibdir = $(libdir)/gcash
pkglibexecdir = $(libexecdir)/gcash
...
CONFIG_HEADER = $(top_builddir)/src/config/gcash-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/gcash-config.h.in \
...
ACLOCAL = ${SHELL} /mnt/GCASH-NEW-JADI/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /mnt/GCASH-NEW-JADI/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /mnt/GCASH-NEW-JADI/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /mnt/GCASH-NEW-JADI/build-aux/missing automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /mnt/GCASH-NEW-JADI/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = gcash
PACKAGE_BUGREPORT = http://bit.gcashs
PACKAGE_NAME = GoCash Core
PACKAGE_STRING = GoCash Core 1.1.1
PACKAGE_TARNAME = gcash
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /mnt/GCASH-NEW-JADI
abs_srcdir = /mnt/GCASH-NEW-JADI
abs_top_builddir = /mnt/GCASH-NEW-JADI
abs_top_srcdir = /mnt/GCASH-NEW-JADI
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /mnt/GCASH-NEW-JADI/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/gcashd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/gcash-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/gcash-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = GoCash-Qt.app
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_gcash_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_gcash_coverage.info test_gcash.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/GoCash-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/gcash-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/gcash-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/gcash-config.h
$(top_srcdir)/src/config/gcash-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/gcash-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/GoCash-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GoCash-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GoCash-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_gcash.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_gcash -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_gcash_filtered.info: test_gcash.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_gcash_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_gcash_coverage.info: baseline_filtered_combined.info test_gcash_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_gcash_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_gcash_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_gcash_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_gcash.coverage/.dirstamp:  test_gcash_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_gcash.coverage/.dirstamp total.coverage/.dirstamp

# GoCash: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_gcash.coverage/ total.coverage/ $(OSX_APP)
