...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GCASHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GCASHMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GCASHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gcash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("GCASHMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running GCASHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadGCASHMiner() exception");
    } catch (...) {
        LogPrintf("ThreadGCASHMiner() exception");
    }

    LogPrintf("ThreadGCASHMiner exiting\n");
}
