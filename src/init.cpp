...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "gcash.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "gcashd.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, gcash, (Darksend, Instantx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GCASH specific functionality (Masternodes, Darksend, InstantX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizegcashamount=<n>", strprintf(_("Keep N GCASH anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The GoCash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that GCASH is running in a usable environment with all
...
#endif
    // Make sure only a single GCASH process is using the data directory.
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeGCASHAmount = GetArg("-anonymizegcashamount", 0);
    if (nAnonymizeGCASHAmount > 999999) nAnonymizeGCASHAmount = 999999;
    if (nAnonymizeGCASHAmount < 2) nAnonymizeGCASHAmount = 2;
    ...
    LogPrintf("Anonymize GCASH Amount %d\n", nAnonymizeGCASHAmount);
