...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::GCASH)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeGCASHAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGCASHAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeGCASHAmount = strAnonymizeGCASHAmount.remove(strAnonymizeGCASHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGCASHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeGCASHAmount * COIN) nMaxToAnonymize = nAnonymizeGCASHAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeGCASHAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeGCASHAmount));
        strAnonymizeGCASHAmount = strAnonymizeGCASHAmount.remove(strAnonymizeGCASHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGCASHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
